 ...
 * LFM2 WebGPU runtime.
 *
 * Shared contracts live in @chomato/schema and the generated codec. This file
 * intentionally keeps only execution-local helpers and scheduling/cache logic.
 */
...
type DispatchParams = Omit<Partial<OpParams>, "mode" | "reserved"> & {
  mode?: Lfm2Mode | 0 | 1 | 2;
};
...
class ParamWriter {
  ...
  alloc(params: DispatchParams): number {
    ...
    const mode = params.mode ?? "prefill";
    u32(typeof mode === "number" ? mode : mode === "decode" ? 1 : mode === "continuation" ? 2 : 0);
    f32(params.f0);
    f32(params.f1);
    u32(params.u0);
    u32(params.u1);
...
export class Lfm2Runtime {
  ...
  private constructor(
    ...
    this.runtimeByteSize = SIZEOF_LlmRuntime;
  ...
  private writeRuntime(state: LlmRuntime): void {
    serializeLlmRuntime(state, this.runtimeView, 0);
  ...
  private async readResult(maxNewTokens: number): Promise<GenerateResult> {
    ...
    try {
      ...
      const state = deserializeLlmRuntime(this.runtimeView, 0);
   ...
   */
  async readTelemetry(): Promise<LlmRuntime | null> {
    ...
    try {
      ...
      return deserializeLlmRuntime(this.runtimeView, 0);
    } finally {
...
// Public type surface remains available from @chomato/webgpu, but the source
// of truth now lives in packages/schema / the generated codec.
export type {
  GpuWeightFormat,
  Lfm2LayerKind,
  Lfm2RuntimeConfig,
  MatmulDispatchArgs,
} from "../../schema/src/model";
export type {
  CacheBlockOptions,
  GenerateOptions,
  GenerateResult,
  GenerateTimings,
} from "../../schema/src/runtime";
export type {
  GpuTensor,
  GpuTensorPage,
  Lfm2RuntimeModel,
...
export type LlmRuntimeState = LlmRuntime;

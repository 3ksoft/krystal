/**
 * LFM2 WebGPU runtime migrated from the original @chomato/lfm2 host runtime.
 *
 * This file intentionally keeps the proven scheduling/cache implementation
 * almost unchanged. Model loading is NOT owned here: the runtime consumes the
 * small structural Lfm2RuntimeModel port below, so GGUF/WQ4 loading can stay in
 * another package until that boundary is decided.
 *
 * The shader backend is still the legacy monolithic WGSL bridge. New shader
 * definitions/programs/passes live in lfm2.ts + pass.ts and can replace the
 * dispatch backend incrementally without moving the scheduler again.
 */
...
export interface GpuTensor {
  ...
  readonly format: GpuWeightFormat;
  readonly dimensions: readonly number[];
  readonly pages: readonly GpuTensorPage[];
  readonly byteLength?: number;
}
...
export interface Lfm2RuntimeConfig {
  ...
  readonly kvHeadsByLayer: readonly number[];
  readonly headDim: number;
  readonly ropeTheta: number;
  readonly vocabSize: number;
  readonly convCacheLength: number;
  readonly normEpsilon: number;
  readonly eosToken: number;
  readonly blockCount: number;
  readonly layers: readonly Lfm2LayerKind[];
  readonly attentionLayerSlots: readonly number[];
}

/** Minimal model/weight-loader contract required by the WebGPU runtime. */
export interface Lfm2RuntimeModel {
  readonly device: GPUDevice;
  readonly config: Lfm2RuntimeConfig;
...
const LLM_RUNTIME_FIELDS = [
  "contextCapacity", "maxNewTokens", "eosToken", "promptTokenCount",
  "position", "generatedCount", "currentToken", "status",
  "telemetryRevision", "lastToken", "errorCode", "pad0",
] as const satisfies readonly (keyof LlmRuntimeState)[];

const LLM_RUNTIME_BYTES = LLM_RUNTIME_FIELDS.length * 4;

function serializeLlmRuntime(view: DataView, state: LlmRuntimeState, offset = 0): void {
  let p = offset;
...
function deserializeLlmRuntime(view: DataView, offset = 0): LlmRuntimeState {
...
function createInitialRuntimeState(options: {
  contextCapacity: number;
  maxNewTokens: number;
  eosToken: number;
  promptTokenCount: number;
...
export class Lfm2Runtime {
  ...
  private constructor(
    device: GPUDevice,
    readonly model: Lfm2RuntimeModel,
    options: {
      contextCapacity: number;
      maxNewTokens: number;
    ...
    if (model.device !== this.device) throw new Error("Lfm2RuntimeModel and Lfm2Runtime must use the same GPUDevice");
  ...
  static async create(
    ...
    const shaderUrl = new URL("./shaders/runtime.legacy.wgsl", import.meta.url);
    ...
    const runtime = new Lfm2Runtime(device, model, resolved, body);
  ...
  private async compile(code: string, matmulEntryPoints: readonly string[]): Promise<void> {
    const module = this.device.createShaderModule({ label: "lfm2.runtime.legacy.wgsl", code });
